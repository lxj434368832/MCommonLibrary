	...
	const QRectF & canvasRect, int from, int to) const
{
	...
	case Lines:
		...
		if (m_iCurrentPoint > from && m_iCurrentPoint < to)
		{
			drawLines(painter, xMap, yMap, canvasRect, from, m_iCurrentPoint - 1);
			drawLines(painter, xMap, yMap, canvasRect, m_iCurrentPoint, to);
		}
		else
		{
