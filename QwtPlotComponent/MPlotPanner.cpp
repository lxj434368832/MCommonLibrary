...
void MPlotPanner::SRange::checkBorder(double & dMin, double & dMax, unsigned int unFactor) const
{
	if (dUpperBound > dLowerBound + 0.001)
	{
		double dRange = dMax - dMin;
		double dRealRange = dUpperBound - dLowerBound;
		/*
		if (dRange > dRealRange)
		{
			dRange = dRealRange + 2 * dRealRange / unFactor;
		}
		double dMargin = dRange / unFactor;*/
		if (dRange > dRealRange)
		{
		...
		double dMargin = dRange / 2;

		if (dMin < dLowerBound - dMargin)
		{
			dMin = dLowerBound - dMargin;
			dMax = dMin + dRange;
		}
		else if (dMax > dUpperBound + dMargin)
		{
			dMax = dUpperBound + dMargin;
			dMin = dMax - dRange;
		}
	}
}
...
void MPlotPanner::moveCanvas(int dx, int dy)
{
	...
	for (int axis = 0; axis < QwtPlot::axisCnt; axis++)
	{
		...
		m_aAxisRange[axis].checkBorder(d1, d2, m_unDivFactor);
